...
class DropModel:
    ...
    def spawn_model(self, model_name):
        for index in range(4):
            ...
            self.initial_pose.position.x = places_target[p][0]
            self.initial_pose.position.y = places_target[p][1]
            self.initial_pose.position.z = places_target[p][2]
            roll = places_target[p][3]
            pitch = places_target[p][4]
            yaw = places_target[p][5]
            ...
            self.spawn_model_prox('target_place_{}'.format(index), self.model_xml, '', self.initial_pose, 'world')
